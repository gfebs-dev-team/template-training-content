...
<template>
  <div class="z-10 box-content flex w-full flex-col items-center justify-end bg-spacecadet xl:gap-4 xl:max-w-[1200px]">
    <div class="grid h-min sm:h-20 w-full grid-cols-[auto_auto_1fr] items-center gap-4 p-4 xl:h-auto">
      ...
      <div class="flex flex-col gap-4 justify-self-end xl:w-2/3">
        ...
        <div class="grid grid-rows-2 sm:grid-rows-1 sm:grid-cols-2 w-full justify-end gap-1 md:gap-4">
          <AppPopover class-name="order-2 sm:order-none"
            ...
            <template #menu>
              <a v-if="glossaryEnable"
                class="font-bold p-2 rounded hover:bg-masblue transition-colors text-xs text-aliceblue md:text-sm xl:w-full xl:max-w-none"
                @click="glossaryState = !glossaryState">Glossary</a>
              <a class="font-bold p-2 rounded hover:bg-masblue transition-colors text-xs text-aliceblue md:text-sm xl:w-full xl:max-w-none"
                @click="openLink('https://gfebs.army.mil/gm')">GFEBS Production PSW</a>
            </template>
          </AppPopover>
          <AppButton
    ...
    <div
      ...
      <RiMenuLine @click="$emit('toggleSidebar')" class="size-6 fill-coolgrey p-1 hover:cursor-pointer xl:size-8">
      </RiMenuLine>
      ...
      <div class="hidden h-12 items-center justify-end gap-4 text-xs text-aliceblue xl:flex">
        <AppButton
          ...
          @click="goPrev()" :disabled="!slides.prev">
          Back
        </AppButton>
        <AppButton
          class="h-fit max-w-fit border-2 border-aliceblue p-2 px-4 disabled:border-coolgrey disabled:text-coolgrey"
          @click="goNext()" :disabled="disable()">
          Next
        </AppButton>
      </div>
    </div>
