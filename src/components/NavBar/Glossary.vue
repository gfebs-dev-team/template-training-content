...
<template>
  <div
    ...
    <div
      ...
      <!-- CLOSE -->
      <RiCloseLine @click="glossaryState = false"
      ...
      <div class="flex flex-col gap-4 justify-between md:flex-row">
        ...
        <div
          ...
          <button class="font-bold p-2 px-3 capitalize text-nowrap hover:opacity-70" :class="[
            { default: search != '' },
          ...
          <div class="alphabet py-1 flex w-full overflow-x-auto">
            <button class="font-bold p-1 pl-3 capitalize text-nowrap hover:opacity-70"
              :class="{ 'text-saffron': activeFilter == '0-9' && search == '' }" @click="activeFilter = '0-9'">
              0-9
            </button>
            <button class="font-bold p-1 capitalize hover:opacity-70"
              :class="{ 'text-saffron': activeFilter == letter && search == '' }" @click="activeFilter = letter"
              v-for="(letter, index) in limitedAlphabet(glossary)" :key="index">
              {{ letter }}
            </button>
          </div>
        </div>
        ...
        <div class="row-start-2 col-span-2">
          <input
            ...
            id="glossary-search" type="search" v-model="search" placeholder="Search" />
        </div>
      </div>
      ...
      <div class="relative flex overflow-x-hidden overflow-auto h-full max-h-fit w-full p-2">
        ...
        <div class="bg-oxfordblue p-2 w-full md:w-2/5">
          <div
            ...
            <template v-for="(item, index) in glossary" :key="item.term">
              <li class="cursor-pointer hover:opacity-70 pb-3" :class="{ 'text-saffron font-bold ': activeIndex == index }"
                @click="setTerm(index)" v-if="getFilter(item.term)">
                {{ item.term }}
              </li>
        ...
        <div
          ...
          <button @click="defArea = false" class="hover:bg-coolgrey h-min hover:bg-opacity-50 rounded p-2 lg:hidden">
          ...
          <div class="flex flex-col gap-2">
            ...
            <p class="whitespace-pre-wrap" :key="activeIndex">{{ glossary[activeIndex].definition }}</p>
          </div>
        </div>
      </div>
    </div>
